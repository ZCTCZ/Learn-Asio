...
void Session::start()
{
    ...
    boost::asio::async_read(m_socket, boost::asio::buffer(m_recv_head->GetData(), HEAD_TOTAL_LEN),
                            boost::asio::bind_executor(
                                m_strand, [shared_this](const boost::system::error_code& ec, size_t transfer_bytes)
                                {
                                    shared_this->handle_read_head(ec, transfer_bytes);
                                }));
}
...
{
    {
        ...
        const auto front_node = m_MsgNode_que.front();
        auto shared_this = shared_from_this();
        boost::asio::post(m_strand, [shared_this, front_node]
        {
            boost::asio::async_write(shared_this->m_socket,
                                     boost::asio::buffer(front_node->GetData(), front_node->Get_Total_Len()),
                                     boost::asio::bind_executor(
                                         shared_this->m_strand,
                                         [shared_this](const boost::system::error_code& ec, size_t)
                                         {
                                             shared_this->handle_write(ec);
                                         }));
        });
...

    {
        ...
        if (!m_MsgNode_que.empty())
        {
            ...
            boost::asio::async_write(m_socket, boost::asio::buffer(front_node->GetData(), front_node->Get_Total_Len()),
                                     boost::asio::bind_executor(
                                         m_strand, [shared_this](const boost::system::error_code& error, size_t)
                                         {
...
void Session::handle_read_head(const boost::system::error_code& ec, size_t transfer_bytes)
{
    ...
    boost::asio::async_read(m_socket, boost::asio::buffer(m_recv_body->GetData(), data_len),
                            boost::asio::bind_executor(
                                m_strand, [shared_this](const boost::system::error_code& ec, size_t transfer_bytes)
                                {
                                    shared_this->handle_read_body(ec, transfer_bytes);
                                }));
}
