...
SendNode::SendNode(const char* msg, MSG_LEN_TYPE len, MSG_TYPE id)
    : MsgNode(HEAD_TOTAL_LEN + len),
      m_msg_id(id)
{
    /// 将 id 转成 网路字节序，并将消息 id 放入报文首部
    const MSG_ID_TYPE id_net = boost::asio::detail::socket_ops::host_to_network_short(
        static_cast<boost::asio::detail::u_short_type>(id));
...
SendNode::~SendNode() = default;

MSG_TYPE SendNode::Get_Msg_Id() const
{
    return m_msg_id;
}
