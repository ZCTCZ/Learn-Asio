...
void Server::start_accept()
{
    ...
    /// 新连接挂载到 #IOServicePool 里的 io_context上
    auto new_session = std::make_shared<Session>(pool->get_ioc(), this);
...
void Server::handle_accept(const boost::system::error_code& ec, std::shared_ptr<Session> new_session)
{
    ...
    {
        /// m_session_map 是临界资源
        /// insert 操作需要加锁，可能其他线程正在从 m_session_map 里删除数据
        std::lock_guard<std::mutex> lock(m_session_map_mtx);
